...
if (isset($_SESSION['user_id'])) {
    ...
    $stmt = $connect->prepare("SELECT * FROM login WHERE id = ?");
    ...
    if ($result->num_rows > 0) {
        ...
        while($row = $result->fetch_assoc()) {
            ...
            echo "<p><strong>Họ và tên:</strong> " . htmlspecialchars($row["username"]) . "</p>";
            echo "<p><strong>Sinh nhật:</strong> " . htmlspecialchars($row["ngaysinh"]) . "</p>";
            echo "<p><strong>Số điện thoại:</strong> " . htmlspecialchars($row["sodienthoai"]) . "</p>";
            echo "<p><strong>Địa chỉ:</strong> " . htmlspecialchars($row["diachi"]) . "</p>";
            echo "<p><strong>Email:</strong> " . htmlspecialchars($row["email"]) . "</p>";
            echo "<p><strong>Ngày đăng kí:</strong> " . htmlspecialchars($row["ngaydangky"]) . "</p>";
            echo "</div>"; 
            echo "<div class='avatar-section'>";
            echo "<form action='' method='post' enctype='multipart/form-data'>";
            echo "<h2>Avatar</h2>";
            if (!empty($row['avatar'])) {
                echo "<div class='avatar-frame'><img src='" . htmlspecialchars($row['avatar']) . "' alt='Avatar' class='avatar'></div><br>";
            } else {
            ...
            $stmt_posts = $connect->prepare("SELECT posts.*, 
            ...
            $result_posts = $stmt_posts->get_result();
                ...
                if ($result_posts->num_rows > 0) {
                    ...
                    // Code for like button and comments...
                    $liked = false;
                    if (isset($_SESSION['user_id'])) {
                        $user_id = $_SESSION['user_id'];
